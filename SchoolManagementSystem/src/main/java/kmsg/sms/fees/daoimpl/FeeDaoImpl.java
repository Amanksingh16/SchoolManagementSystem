...
import kmsg.sms.fees.daoint.FeeDaoInt;
import kmsg.sms.fees.mapper.ClassFeesMapper;
import kmsg.sms.fees.mapper.FeeClassHeadMapper;
import kmsg.sms.fees.mapper.FeeClassStudentMapper;
import kmsg.sms.fees.mapper.FeeHeadCollectionMapper;
import kmsg.sms.fees.mapper.FeeHeadMapper;
...
public class FeeDaoImpl implements FeeDaoInt, SMSLogger
{
	...
	public Map<String, Object> selectFeeClassStudents(int classId)
	{
		...
		String SQL = " SELECT" + 
				...
				" FROM "+schoolId+"_student s" + 
				" LEFT JOIN "+schoolId+"_student_sibling ss ON ss.student_id = s.student_id" + 
		...
		try {
			...
			{
				data.put(Constants.STATUS,Constants.FAILURE);
				data.put(Constants.MESSAGE,"No Students available for this class");
				return data;
			}
	...
	{
		final String SQL = "DELETE FROM "+schoolId+"_fee_head_collections"
				+ " WHERE fee_head_id = ?";
	...
	{
		final String SQL = "DELETE FROM "+schoolId+"_class_fee_head_collections"
				+ " WHERE class_id = ?";
	...
	{
		final String SQL = "DELETE FROM "+schoolId+"_student_fee_head_collections"
				+ " WHERE student_id = ?";

		try {
		    template.update(SQL,new Object[] {studentId});
		}
		catch(Exception e) {
	...
	public Map<String, Object> selectFeeHeadCollections(int feeHeadId) 
	{
		...
		final String SQL = 
				...
				+ " amount,"
				+ " remarks "
				+ " FROM "+schoolId+"_fee_head_collections" + 
				"   WHERE fee_head_id = ?" ;
